...
class GeminiService:
    ...
    @staticmethod
    def _generate(prompt: str, system_instruction: str, temperature: float = 0.5, max_tokens: int = 1500) -> str:
        """Shared helper to call the Gemini API and return raw text"""
...
"""
            response_text = GeminiService._generate(
                prompt,
                "You are a resume analyzer. Extract skills from resumes.",
                temperature=0.3,
                max_tokens=1500
            )
            ...
            print(f"Gemini API Error (extract_skills): {e}")
            return []
...
"""
            response_text = GeminiService._generate(
                prompt,
                "You are an expert interviewer evaluating technical answers.",
                temperature=0.5,
                max_tokens=1000
            )
            ...
            print(f"Gemini API Error (evaluate_answer): {e}")
            return {
                "score": 0,
...
"""
            response_text = GeminiService._generate(
                prompt,
                "You are a career coach creating improvement plans.",
                temperature=0.6,
                max_tokens=1500
            )
            ...
            print(f"Gemini API Error (generate_improvement_plan): {e}")
            return {}
